...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	IplImage* g_Tedge = cvCreateImage( g_ImageSize, IPL_DEPTH_32F, 1 );
	cvConvertScale( g_pSrcImage, g_edge, 1, 0 );	

	//平滑edge map
	//cout << "平滑edge map" << endl;
	cvSmooth( g_edge, g_edge, CV_MEDIAN );
	cvSmooth( g_edge, g_edge, CV_GAUSSIAN );

	//求edge map的梯度
	//cvSobel( g_edge, g_Tedge, 1, 1, 5 );
	cvLaplace( g_edge, g_Tedge, 1 );
	cvConvertScale( g_Tedge, g_edge, 1 );
