...
void GetGradient( CvMat* src, CvMat* u, CvMat* v )
{
	...
	for ( int i = 1; i < l_iHeight; i ++ )
	{
		...
		double l_fAfter = 0.0;
		double l_fBefore = 0.0;

		l_fAfter = cvmGet( src, i + 1, 0 );
		l_fBefore = cvmGet( src, i - 1, 0 );
		cvmSet( v, i, 0, 0.5 * ( l_fAfter - l_fBefore ) );

		l_fAfter = cvmGet( src, i + 1, l_iWidth );
		l_fBefore = cvmGet( src, i - 1, l_iWidth );
		cvmSet( v, i, l_iWidth, 0.5 * ( l_fAfter - l_fBefore ) );
	}//for ( int i = 1; i < l_iHeight; i ++ )
	...
	for ( int i = 1; i < l_iWidth; i ++ )
	{
		...
		l_fAfter = cvmGet( src, 0, i + 1 );
		l_fBefore = cvmGet( src, 0, i - 1 );
		cvmSet( u, 0, i, 0.5 * ( l_fAfter - l_fBefore ) );

		l_fAfter = cvmGet( src, l_iHeight, i + 1 );
		l_fBefore = cvmGet( src, l_iHeight, i - 1 );
		cvmSet( u, l_iHeight, i, 0.5 * ( l_fAfter - l_fBefore ) );
	}//for ( int i = 1; i < l_iWidth; i ++ )
	...
	for ( int j = 1; j < l_iWidth; j ++ )
	{
		for ( int i = 1; i < l_iHeight; i ++ )
		{
			...
			double l_fBefore = 0;
			//求u
			l_fAfter = cvmGet( src, i, j + 1 );
			l_fBefore = cvmGet( src, i, j - 1 );
			cvmSet( u, i, j, 0.5 * ( l_fAfter - l_fBefore ) );
			//求v
			l_fAfter = cvmGet( src, i + 1, j );
			l_fBefore = cvmGet( src, i - 1, j );
			cvmSet( v, i, j, 0.5 * ( l_fAfter - l_fBefore ) );
		}//for ( int i = 1; i < l_iHeight; i ++ )
	}//for ( int j = 1; j < l_iWidth; j ++ )
}