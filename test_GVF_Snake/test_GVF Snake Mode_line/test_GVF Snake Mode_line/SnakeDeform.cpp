				 ...
				 int iter )
{
	...
	double l_dA = beta;
	double l_dB = - alpha - 4.0 * beta;
	double l_dC = 2.0 * alpha + 6.0 * beta;
	double l_dD = - alpha - 4.0 * beta;
	double l_dE = beta;
	...
	for ( int j = 0; j < l_ContourSize; j ++ )
	{
		...
		cvmSet( A, l_ap, j, l_dA );
		cvmSet( A, l_bp, j, l_dB );
		cvmSet( A, l_cp, j, l_dC );
		cvmSet( A, l_dp, j, l_dD );
		cvmSet( A, l_ep, j, l_dE );
	}//for ( int j = 0; j < l_ContourSize; j ++ )
	...
	{
		double l_dGamma = cvmGet( A, i, i ) + gamma;
		cvmSet( A, i, i, l_dGamma );
	}//for ( int i = 0; i < l_ContourSize; i ++ )
	cvInvert( A, A );
	...
	for ( int k = 0; k < iter; k ++ )
	{
		...
		for ( int i = 0; i < l_ContourSize; i ++ )
		{
			...
			double l_dVfx = cvmGet( u, l_iYi, l_iXi );
			double l_dVfy = cvmGet( v, l_iYi, l_iXi );

			cvmSet( l_mVfx, i, 0, l_dVfx );
			cvmSet( l_mVfy, i, 0, l_dVfy );
		}//for ( int i = 0; i < l_ContourSize; i ++ )
		//   x = invAI * (gamma* x + kappa*vfx);
		//   y = invAI * (gamma* y + kappa*vfy);
