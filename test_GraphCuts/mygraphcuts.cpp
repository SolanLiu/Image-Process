...
void MyGraphCuts::setImageAndWinName(const Mat& _image, const string& _winName)
{
...
void MyGraphCuts::showImage() const
{
...
void MyGraphCuts::setRectInMask()
{
    assert(!mask.empty());
...
void MyGraphCuts::setLblsInMask(int flags, Point p, bool isPr)
{
...
void MyGraphCuts::mouseClick(int event, int x, int y, int flags, void*)
{
...
void MyGraphCuts::on_segementationG_clicked()
{
    Mat image = imread("F://Qt5.8.0//myProcedure//test_GraphCuts//testImage.bmp", 1);
    if(image.empty())
    {
